...
Ψ(n,m,t) = 2 * βᵢ(n,t) - 2 * im * P(n,m) * β(n,t) * exp(-im * Ω(n) * t) +  im * C(n,m) * conj(β(m,t)) * exp(-im * Ω(n) * t) - im * Δₚ(n,m,t)/D(n,m,t) * Φ(n,m,t)
...
time_evol(n,m,t) = 1/(μ(n)*μ(m)) * exp(im * (ψ(n,t)-ψ(m,t))) /((pi^4) * sqrt((1/2)^8 * A(n,m) * B(n,m) * A_σ(n,m) * B_σ(n,m) * D(n,m,t) * E(n,m,t) * H(n,m,t) * G(n,m,t)))* exp(1/2 * ϕ_final(n,m,t)^2(4 * H(n,m,t))) * exp(-1/2 * C(n,m) * β(n,t) * conj(β(m,t)) - Ψ(n,m,t)^2/(4 * E(n,m,t)) - Φ(n,m,t)^2/(4 * D(n,m,t)) - 1/2 *(abs(β(n,t))^2 + abs(β(m,t))^2) - 1/2 * P(n,m) * β(n,t)^2 -1/2 * Q(n,m) * β(m,t)^2)


println(time_evol(2,3,1e-3))